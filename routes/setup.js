...
try {
    if (!config.enableTagging) {
      return res.status(200).send('Tagging disabled');
    }
    const structural = config.validate();
    if (!structural.valid) {
      return res.status(503).json({ error: 'Invalid configuration', details: structural.errors });
    }

    const userId = await paperlessService.getOwnUserID();
    if (!userId) {
      console.error('Failed to get own user ID. Abort scanning.');
      return res.status(503).send('Cannot resolve Paperless user for tagging');
    }
    ...
    console.error('[ERROR] in /api/scan/now:', error);
    if (!res.headersSent) {
...
router.get('/setup', (req, res) => {
  res.status(410).type('text/plain').send(
    'Setup wizard removed. Configure with environment variables; see .env.example.',
  );
});

router.post('/setup', express.json(), (req, res) => {
  res.status(410).json({ error: 'Setup wizard removed. Configure with environment variables.' });
});
...
async function processQueue(customPrompt) {
  ...
  try {
    if (!config.enableTagging || !config.validate().valid) {
      return;
    }

    const userId = await paperlessService.getOwnUserID();
    if (!userId) {
      console.error('Failed to get own user ID. Abort queue processing.');
      return;
    }
...
router.get('/settings', async (req, res) => {
  ...
  const isConfigured = configFile.validate().valid;
  let config = {
  ...
  res.render('settings', { 
    ...
    settingsError: !isConfigured ? 'Configuration is incomplete or invalid. Set required environment variables (see .env.example).' : undefined
  });
});

router.post('/settings', express.json(), (req, res) => {
  res.status(410).json({ error: 'Persisting settings via API was removed. Use environment variables.' });
});
